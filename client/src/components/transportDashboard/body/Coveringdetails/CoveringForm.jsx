...
//     .test(
//       "is-valid",
//       "Please enter 5 numbers separated by spaces",
//       (value) => {
//         const numbers = value.split(" ").map((num) => parseInt(num));
//         return numbers.length === 5 && numbers.every((num) => !isNaN(num));
//       }
//     ),
...
const CoveringForm = ({ handleSubmit, initialData }) => {
  ...
  const formik = useFormik({
    ...
    //   total_coverings: initialData ? initialData.total_coverings : "",
  ...
  const calculateTotalCoverings = (values) => {
    const totalCoverings = values.total_coverings || ""; // Ensure total_coverings is not undefined or null
    const coverings = totalCoverings.split(" ").map((num) => parseInt(num));
    const total = coverings.reduce((acc, curr) => acc + curr, 0);
    return total;
  };
          ...
          <div className="col">
            ...
            <div className="mb-3">
              <label htmlFor="total_coverings" className="form-label">
                Total Coverings (Enter 5 numbers separated by spaces)
              </label>
              <input
                type="text"
                className="form-control"
              ...
              <div className="mb-2">
                Total:{" "}
                {calculateTotalCoverings(formik.values.total_coverings)}
              </div>
              {formik.touched.total_coverings &&
                formik.errors.total_coverings && (
                  <div className="error">{formik.errors.total_coverings}
                  </div>
                )}
            </div>
            <div className="mb-3">
