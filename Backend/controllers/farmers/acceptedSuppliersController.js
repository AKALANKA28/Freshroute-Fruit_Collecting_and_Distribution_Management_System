...
const addAcceptedSupplier = async (req, res) => {
  ...
  try {
    const newAcceptedSupplier = await AcceptedSupplier.create({ name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl });
...
const updateAcceptedSupplier = async (req, res) => {
  ...
  const { name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl } = req.body;
  try {
    await AcceptedSupplier.findByIdAndUpdate(id, { name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl });
