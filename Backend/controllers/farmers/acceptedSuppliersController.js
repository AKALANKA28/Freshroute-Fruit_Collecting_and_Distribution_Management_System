...
const addAcceptedSupplier = async (req, res) => {
  ...
  try {
    const newAcceptedSupplier = await AcceptedSupplier.create({ name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl, joinRequestId });
...
const updateAcceptedSupplier = async (req, res) => {
  ...
  const { name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl, joinRequestId } = req.body;
  try {
    await AcceptedSupplier.findByIdAndUpdate(id, { name, email, mobile, city, NIC, landAddress, fieldArea, landDeedUrl, joinRequestId });
